...
// New Function to search for multiple users
const fetchMultipleProfiles = async (event) => {
    ...
    if (event.key === "Enter" && query !== "") {
        try {
            ...
            if (data.items && data.items.length > 0) {
                profileBox.innerHTML = ""; // Clear previous results
                data.items.forEach((profile) => {
                    profileBox.innerHTML += profileGenerator(profile);
                });
                loading.innerText = "";
            } else {
                loading.innerHTML = "No users found.";
                loading.style.color = "red";
                profileBox.innerHTML = "";
            }
        } catch (error) {
            console.error(error);
            loading.innerText = "Failed to fetch profiles.";
            loading.style.color = "red";
        }
    } else if (query === "") {
        profileBox.innerHTML = "";
        loading.innerText = "Please enter a search query.";
        loading.style.color = "red";
    }
};

// Usage for multiple profiles
searchInputEl.addEventListener("keypress", fetchMultipleProfiles);
